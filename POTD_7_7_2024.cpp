...
class Solution {
  public:
    bool findAncestors(Node* root, int target, vector<int>& ancestors) {
        ...
        // If root is the target node, no ancestors to add
        if (root->data == target) {
            return true;
        }
        
        // Recursively find target in left and right subtrees
        if (findAncestors(root->left, target, ancestors) ||
            findAncestors(root->right, target, ancestors)) {
            // If target found in left or right subtree, root is ancestor
    ...
    vector<int> Ancestors(Node *root, int target) {
        ...
        // Find ancestors starting from root
